...
def productDetails(request, name):
    try:    
        product = Product.objects.filter(name = name, status = 1).first()
...
def create_order(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            context = {
                'callback_url': os.getenv('HTTP') + os.getenv('DEV_URL') + '/callback',
...
def callback(request):
    return JsonResponse({'status': request.POST})
    if "razorpay_signature" in request.POST:
        ...
        # order.status = ERROR
        # order.save()
        # return render(request, "shop/order/callback.html", context={"status": ''})
